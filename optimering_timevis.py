...
def inntekt_halvD(model):
    return (model.innbydd_effekt_N) * model.markedspris + (model.innbydd_effekt_opp) * model.markedspris_D # [EUR]

# + model.innbydd_effekt_ned
...
def reservert_P_N(model):
    return model.innbydd_effekt_N * 1.34 # [MW]

def reservert_P_Dopp(model):
    return model.innbydd_effekt_opp# + (model.innbydd_effekt_ned) * 0.2 # [MW]

#def reservert_P_Dned(model):
#    return model.innbydd_effekt_ned + (model.innbydd_effekt_opp) * 0.2 # [MW]
...
def make_model(markedspris, markedspris_D, frekvensavvik, SOC):
    model = ConcreteModel()
...
#   model.innbydd_effekt_ned = Var(initialize=0.2) # MW   
    ...
    #model.markedspris_D = Param(initialize=markedspris_D / 2, mutable=True) # EUR/MW
...
#    model.c.add(model.innbydd_effekt_N * 1.07 + model.innbydd_effekt_ned <= installert_kap) #* snitt_volum_pris(model)
    ...
    model.c.add(reservert_effekt_oppregulering(model) <= installert_kap) # [MW]
    model.c.add(reservert_effekt_nedregulering(model) <= installert_kap) # [MW]
    model.c.add(reservert_effekt_oppregulering(model) <= 3.2) # [MW]
    model.c.add(reservert_effekt_nedregulering(model) <= 3.2)    

    model.c.add(reservert_energi_oppregulering_batteri(model) <= installert_kap_MWh * 0.5 * (1 - (model.SOC - 0.5))) # [MWh], 
    model.c.add(reservert_energi_nedregulering_batteri(model) <= installert_kap_MWh * 0.5 * (1 - (0.5 - model.SOC))) # [MWh], 




    # Objective ############################################################
    model.obj = Objective(expr= (inntekt_halvD(model) - kostnad(model)), sense=maximize)
    

    return model, solver

def optimal_fortjeneste(pris, pris_d, avvik, SOC):
    # create model
    model, solver = make_model(pris, pris_d, avvik, SOC)
    ...
    return value(model.innbydd_effekt_N), value(model.innbydd_effekt_opp), value(model.bud_pris_N), value(model.obj), value(kostnad(model)), value(model.innbydd_effekt_N * model.markedspris), value(inntekt_halvD(model)), value(SOC_delta(model) + model.SOC)
...
# ned,
        # Solve the model
        effekten, opp, budprisen, overskuddet, kostnad_batteri, inntekt_batteri, inntekt_total, SOC_etter = optimal_fortjeneste(pris, pris_d, avvik, SOC_loop)
        SOC_loop = SOC_etter
