...
def C_rate(model):
    ...
    return (abs_avvik + c + d) * model.innbydd_effekt_N / installert_kap # [MW/MW]

def ladning(model):
    abs_volum, volum = volum_effekt_N(model)
    c, a = volum_effekt_D_opp(model)
    d, a = volum_effekt_D_ned(model)
    return model.innbydd_effekt_N * (abs_volum + c + d) * 60 / spenning  # [Ah]
...
def inntekt(model):
    return (model.innbydd_effekt_N + model.deltakelse_vann_N * kap_vann) * model.markedspris + (model.innbydd_effekt_opp + model.innbydd_effekt_ned + (model.deltakelse_vann_opp + model.deltakelse_vann_ned) * kap_vann) * model.markedspris_D # [EUR]

def kostnad_vann(model):
    return (model.spotpris) * (model.deltakelse_vann_N + model.deltakelse_vann_opp + model.deltakelse_vann_ned) * kap_vann # [EUR]
...
def SOC_delta(model):
    ...
    return (volum + c + d) / installert_kap 
...
def reservert_P_Dned(model):
    return model.innbydd_effekt_ned + model.deltakelse_vann_ned * kap_vann + (model.innbydd_effekt_opp + model.deltakelse_vann_opp * kap_vann) * 0.2 # [MW]
...
def make_model(markedspris, markedspris_D, frekvensavvik, spotpris, SOC):
    ...
    model.c.add(model.deltakelse_vann_opp + model.deltakelse_vann_ned + model.deltakelse_vann_N <= 1) # [MW]
    ...
    # Objective ############################################################
    model.obj = Objective(expr= (inntekt(model) - kostnad(model) - kostnad_vann(model)), sense=maximize)
...
def optimal_fortjeneste(pris, pris_d, avvik, spotpris, SOC):
    ...
    return value(model.innbydd_effekt_N), value(model.innbydd_effekt_opp),  value(model.innbydd_effekt_ned), value(model.bud_pris_N), value(model.obj), value(model.deltakelse_vann_N), value(kostnad(model)), value(model.innbydd_effekt_N * model.markedspris), value(inntekt(model)), value(SOC_delta(model) + model.SOC), value(kostnad_vann(model))
...
def run_model_dag(csv_or_pandas, csv = True, SOC = 0.5):
    ...
    for idx, row in dag_df.iterrows():
        ...
        # Solve the model
        effekten, opp, ned, budprisen, overskuddet, deltakelsen_vann, kostnad_batteri, inntekt_batteri, inntekt_total, SOC_etter, kostnad_vann = optimal_fortjeneste(pris, pris_d, avvik, spotpris, SOC_loop)
        SOC_loop = SOC_etter
