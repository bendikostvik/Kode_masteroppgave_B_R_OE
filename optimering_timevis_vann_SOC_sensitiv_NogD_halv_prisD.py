...
def optimal_fortjeneste(pris, pris_d, avvik, spotpris, SOC):
    ...
    return value(model.innbydd_effekt_N), value(model.innbydd_effekt_opp),  value(model.innbydd_effekt_ned), value(model.bud_pris_N), value(model.obj), value(model.deltakelse_vann_N), value(kostnad(model)), value(model.innbydd_effekt_N * model.markedspris), value(inntekt_halvD(model)), value(SOC_delta(model) + model.SOC), value(kostnad_vann(model))
